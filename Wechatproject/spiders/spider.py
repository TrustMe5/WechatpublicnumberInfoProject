...
class WechatSpider(BaseSpider):
    ...
    for username in data['4']:
        if username is not None:
            print 'username:',username
            start_urls.append("http://weixin.sogou.com/weixin?type=1&query=%s&page=1"%username)
    ...
    def parse2(self, response):
        soup = BeautifulSoup(response.body)
        tag=soup.find("div",attrs={"class":"profile_desc_value"})
        intro ="".join(tag.get_text()) 
